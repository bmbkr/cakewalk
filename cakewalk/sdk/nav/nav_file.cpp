...
namespace nav_mesh {
	...
	std::vector< Vector > nav_file::find_path( Vector from, Vector to ) {
		...
		for ( std::size_t i = 0; i < path_area_ids.size( ); i++ ) {
			for ( auto& area : m_areas ) {
				if ( area.get_id( ) == std::uint32_t( path_area_ids[ i ] ) ) {
					const Vector center = area.get_center();
					path.push_back( Vector(center.x, center.y, center.z) );
					break;
				}
			}
		}
	...
	std::vector< nav_area* > nav_file::find_path(int fromId, int toId) {
		auto start = reinterpret_cast< void* >( fromId );
		auto end = reinterpret_cast< void* >( toId );
		...
		std::vector< nav_area* > path = { };
		for ( std::size_t i = path_area_ids.size(); i > 0; i-- ) {
			for ( auto& area : m_areas ) {
				if ( area.get_id( ) == std::uint32_t( path_area_ids[ i-1 ] ) ) {
					path.push_back( &area );
					break;
				}
			}
	...
	nav_area* nav_file::GetNearestArea(Vector vPosition) {
		try
		{
			return &this->get_area_by_position(vPosition);
		}
	...
	std::string nav_file::GetNameOfPlace(uint16_t placeIdx) {
		if(placeIdx + 1 < m_places.size())
			return this->m_places[placeIdx];
	...
	nav_area& nav_file::get_area_by_position( Vector position ) {
		for ( auto& area : m_areas ) {
